...
The API provides a healthcheck endpoint for monitoring server availability and retrieving system information.

<Card title="GET /v1/healthcheck">

Returns the current status of the API server along with environment and version information.
...
The API supports Cross-Origin Resource Sharing (CORS) for trusted origins. This allows web applications from specified domains to make requests to the API.

<Callout type="info" title="Trusted Origins">
  By default, CORS is configured via the `--cors-trusted-origins` command-line flag. Origins must be explicitly whitelisted to access the API from a browser.
</Callout>

**Allowed Methods**: `OPTIONS`, `PUT`, `PATCH`, `DELETE`  
**Allowed Headers**: `Authorization`, `Content-Type`

**Example**: Start the server with trusted origins:
```bash
go run ./cmd/api --cors-trusted-origins="http://localhost:3000 https://example.com"
```
